...
folder_paths <- list.files(path = "Data/Scenes/2020_2021/MNDWI_Scenes/Test", full.names = TRUE)
...
Water_area_per_Scene <-  lapply(folder_paths, function(folder_path){
  ...
  # cropping the MNDWI raster to the wetlands area
  
  Wetland_rasters <- lapply(bound_list, function(b) {
    
    
    rasters <- raster::crop(x = MNDWI, y = sf::as_Spatial(b))
  ...
  #Cropping MNDWI raster to GPS Areas
...
write.csv(output, file = "Otsu_2020_2021_custom_buffers_and_GPS_and_MNDWI_Band12.csv")
...
file_path <- "NDVI_Test.tif"

# Export the raster to a file (GeoTIFF format in this case)
writeRaster(NDVI, filename = file_path, format = "GTiff", overwrite = TRUE)
