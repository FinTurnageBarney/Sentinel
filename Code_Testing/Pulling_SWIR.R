...
for (x in 1:nrow(result)) {
  ...
  folder_name <- paste0(getwd(), "/Data/Scenes/2018_2019/MNDWI_Scenes/Cloudcover40/", result$Date[x])
  ...
  for (i in grep("B11_20m|B12_20m",bands)) {
  ...
  for (band_node in band_path) {
    ...
    # Save the product
    outfile <- file.path(paste0(getwd(), "/Data/Scenes/2018_2019/MNDWI_Scenes/Cloudcover40/", result$Date[x]), band_node[6])
    writeBin(content(file, "raw"), outfile)
