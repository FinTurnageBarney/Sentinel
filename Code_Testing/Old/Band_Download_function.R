...
#define a folder to save your data ******* MUST BE WITHIN WORKING PROJECT DIRECTORY*******
output_path <- "/Data/Scenes/2018_2019/Cloudcover40/"
...
result <-  subset(result, result$overlap_pct > .95)
...
for (x in 1:nrow(result)) {
  ...
  folder_name <- paste0(getwd(), output_path , result$Date[x])
  ...
  for (band_node in band_path) {
    ...
    # Save the product
    outfile <- file.path(paste0(getwd(), output_path, result$Date[x]), band_node[6])
    writeBin(content(file, "raw"), outfile)
