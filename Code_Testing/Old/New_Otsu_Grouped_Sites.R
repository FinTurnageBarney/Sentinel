...
folder_path <- list.files(path = "Data/Scenes/2020_2021/All_Scenes/test", full.names = TRUE)
...
Otsu_Water_area_per_Scene <-  lapply(folder_paths, function(folder_path){
  ...
  E_Wetland_Group_Areas <- lapply(E_WL_Group, function(E){
    ...
    # calculating the area based on the otsu threshold
    (sum(raster::values(Wl_NDWI) > Otsu_Thresholds$E_WL_Group_Bbox , na.rm = TRUE))*100
    
  })
  #Naming The sites again
  names(E_Wetland_Group_Areas) <- E_WL_Group_Names
  ...
  C_Wetland_Group_Areas <- lapply(C_WL_Group, function(C){
    ...
    # calculating the area based on the otsu threshold
    (sum(raster::values(Wl_NDWI) > Otsu_Thresholds$C_Wl_Group_Bbox , na.rm = TRUE))*100
    
  })
  #Naming The sites again
  
  names(C_Wetland_Group_Areas) <- C_WL_Group_Names
  
  
  Otsu_Group_result <- c(E_Wetland_Group_Areas, C_Wetland_Group_Areas)
...
names(Otsu_Water_area_per_Scene) <- folder_paths
...
output <- do.call("rbind", lapply(folder_paths, function(f){
  ...
  s = unlist(Otsu_Water_area_per_Scene[[f]])
